...
{
    public class Marvelpi
    {
        private string BaseUri = "http://gateway.marvel.com";
        private string _publicKey;
        private string _privateKey;

        public Marvelpi(string publicKey, string privateKey)
        {
        ...
        public async Task<Wrapper<T>> ExecuteRequestAsync<T>(string api, int? limit = null, int? offset = null, IEnumerable<APIFilter> filters = null)
        {
            string timeStamp = GetTimeStamp();
            var parameters = new List<KeyValuePair<string, string>> {
                new KeyValuePair<string, string>("ts", timeStamp),
                new KeyValuePair<string, string>("apikey", _publicKey),
                new KeyValuePair<string, string>("hash", GetHash(timeStamp, _privateKey, _publicKey))
            };
            if (limit.HasValue)
            {
